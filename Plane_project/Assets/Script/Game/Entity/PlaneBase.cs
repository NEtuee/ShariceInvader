...
public abstract class PlaneBase : Collisionable {
	...
	public class DelayItem
	{
		...
		public void Set(int a, float t , ObjectBase at)
		{
			attack = a;
			timer = t;
			attacker = at;
		...
		public DelayItem(int a, float t, ObjectBase at)
		{
			Set(a,t,at);
		}
	}
	...
	public void BasicInitialize()
	{
		...
		_fallTimer = UnityEngine.Random.Range(1f,1.5f);
	...
	public virtual void AfterHit(ObjectBase target){}

	public void AddDelayAttackList(int attack, float time, ObjectBase attacker, hitEventDelegate hitEvent)
	{
		DelayItem item = null;
		...
		if(_delayPool.Count != 0)
		{
			...
			item.Set(attack,time,attacker);
		}
		else
		{
			item = new DelayItem(attack,time,attacker);
		}
	...
	public void DelayAttackProgress(float deltaTime)
	{
		for(int i = 0; i < _delayHitList.Count;)
		{
			if(_delayHitList[i].progress(deltaTime))
			{
				...
				Hit(_delayHitList[i].attack,_delayHitList[i].attacker);
...
//		direction = _velocity.x == 0f ? direction : (_velocity.x > 0 ? Define.Direction.Right : Define.Direction.Left);
	...
	public virtual void Explosion()
	{
		if(_fall)
		{
			Vector3 randDir = MathEx.RandomVector3(-1f,1f,-1f,1f).normalized;
	...
	public void TrailSetUp(Vector2 pos, string material, float time, float startWidth, float endWidth)
	{
		...
		_trailMat.SetFloat("_random",UnityEngine.Random.Range(.5f,1.5f));
	...
	public void PhysicsDebugSetup()
	{
		...
		directionViewer.material = ResourceManager.GetInstance().GetPixelSnapMaterial();
		velocityViewer.material = ResourceManager.GetInstance().GetPixelSnapMaterial();
